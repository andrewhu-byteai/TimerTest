...
# Usage:
#   caos_list_unique(<list_variable> [<list_variable>] [...])
macro(caos_list_unique)
  foreach(__lst ${ARGN})
    if(${__lst})
...
# Usage:
#   caos_clear_vars(<variables_list>)
macro(caos_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var})
  endforeach()
endmacro()
...
# Usage:
#   caos_string_unique(<string_variable>)
function(caos_string_unique __string)
  if(${__string})
    set(__list ${${__string}})
...
# Usage:
#   caos_print_list(<list>)
function(caos_print_list)
  foreach(e ${ARGN})
    message(STATUS ${e})
  endforeach()
endfunction()
...
# Usage:
#   caos_merge_flag_lists(out_variable <list1> [<list2>] [<list3>] ...)
function(caos_merge_flag_lists out_var)
  set(__result "")
  foreach(__list ${ARGN})
...
# Usage:
#   caos_convert_absolute_paths(<list_variable>)
function(caos_convert_absolute_paths variable)
  set(__dlist "")
...
# Usage:
#   caos_parse_header(<file> <define1> <define2> <define3> ..)
macro(caos_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
# Usage:
#   caos_parse_header_single_define(<library_name> <file> <define_name>)
function(caos_parse_header_single_define LIBNAME HDR_PATH VARNAME)
  set(${LIBNAME}_H "")
  if(EXISTS "${HDR_PATH}")
...
# Usage:
#   caos_option(<option_variable> "doc string" <initial value or boolean expression> [IF <condition>])
function(caos_option variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage:
#   caos_compare_lists(<list_variable> <list2_variable> [description])
function(caos_compare_lists list1 list2 desc)
  set(__list1 ${${list1}})
  set(__list2 ${${list2}})
  list(SORT __list1)
  list(SORT __list2)
...
# Usage:
#   caos_warnings_disable(<CMAKE_[C|CXX]_FLAGS[_CONFIGURATION]> -Wshadow /wd4996 ..,)
macro(caos_warnings_disable)
  set(_flag_vars "")
  set(_msvc_warnings "")
  set(_gxx_warnings "")
  ...
  caos_clear_vars(_flag_vars _msvc_warnings _gxx_warnings)
endmacro()

################################################################################################
# Helper function get current definitions
# Usage:
#   caos_get_current_definitions(<definitions_variable>)
function(caos_get_current_definitions definitions_var)
...
# Usage:
#   caos_get_current_cflags(<cflagslist_variable>)
function(caos_get_current_cflags cflags_var)
  get_property(current_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  caos_convert_absolute_paths(current_includes)
  caos_get_current_definitions(cflags)

  foreach(i ${current_includes})
    list(APPEND cflags "-I${i}")
  endforeach()

  caos_list_unique(cflags)
...
# Usage:
#   caos_parse_linker_libs(<caos_LINKER_LIBS_var> <directories_var> <libflags_var> <frameworks_var>)
function(caos_parse_linker_libs caos_LINKER_LIBS_variable folders_var flags_var frameworks_var)
  ...
  foreach(list_elem ${${caos_LINKER_LIBS_variable}})
  ...
  caos_list_unique(libflags folders)
...
# Usage:
#   caos_detect_darwin_version(<version_variable>)
function(caos_detect_darwin_version output_var)
  if(APPLE)
...
function(caos_source_group group)
...
#################################################################################################
# Simple test
function(caos_add_test file suffix)
	string(REGEX REPLACE ".*/" "" file0 ${file})
	string(REPLACE "${suffix}" "" file_exe ${file0})
	add_executable(${file_exe} ${file})
	target_link_libraries(${file_exe} ${ARGN} ${GTEST_LIBRARY})
